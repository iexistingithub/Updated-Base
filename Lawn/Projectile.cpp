...
void Projectile::ProjectileInitialize(int theX, int theY, int theRenderOrder, int theRow, ProjectileType theProjectileType)
{
	...
	else if (mProjectileType == ProjectileType::PROJECTILE_LOBBED_PUFF)
	{
		TodParticleSystem* aParticle = mApp->AddTodParticle(mPosX + 13.0f, mPosY + 13.0f, 400000, ParticleEffect::PARTICLE_PUFFSHROOM_TRAIL);
		AttachParticle(mAttachmentID, aParticle, 13.0f, 13.0f);
	}
...
bool Projectile::CantHitHighGround()
{
	...
	return (
		...
		mProjectileType == ProjectileType::PROJECTILE_SOUNDWAVE ||
		mProjectileType == ProjectileType::PROJECTILE_REDWAVE ||
		mProjectileType == ProjectileType::PROJECTILE_TOXIC_PEA
		) && !mOnHighGround;
}
...
unsigned int Projectile::GetDamageFlags(Zombie* theZombie)
{
	...
	if (mProjectileType == ProjectileType::PROJECTILE_SNOWPEA || mProjectileType == ProjectileType::PROJECTILE_WINTERMELON || mProjectileType == ProjectileType::PROJECTILE_LOBELIA_PETAL)
	{
...
void Projectile::UpdateLobMotion()
{
	...
	if (mProjectileAge > 20)
	{
		...
		else if (mProjectileType == ProjectileType::PROJECTILE_CABBAGE || mProjectileType == ProjectileType::PROJECTILE_LOBELIA_PETAL)
		{
	...
	{
		mBoard->KillAllZombiesInRadius(mRow, mPosX + 80, mPosY + 40, 115, 1, true, false, mDamageRangeFlags);
		DoImpact(nullptr);
	}
	else
	{
...
void Projectile::DoImpact(Zombie* theZombie)
{
	...
	if (mProjectileType == ProjectileType::PROJECTILE_SOUNDWAVE)
	{
		if (theZombie)
		{
...
void Projectile::Update()
{
	...
	if (mProjectileType == ProjectileType::PROJECTILE_PEA || 
		...
		mProjectileType == ProjectileType::PROJECTILE_LOBBED_PUFF ||
		mProjectileType == ProjectileType::PROJECTILE_LOBELIA_PETAL ||
		mProjectileType == ProjectileType::PROJECTILE_TOXIC_PEA ||
		mProjectileType == ProjectileType::PROJECTILE_SOUNDWAVE ||
		mProjectileType == ProjectileType::PROJECTILE_REDWAVE ||
		mProjectileType == ProjectileType::PROJECTILE_SPIKE)
...
void Projectile::DrawShadow(Graphics* g)
{
	...
	case ProjectileType::PROJECTILE_TOXIC_PEA:
	case ProjectileType::PROJECTILE_SOUNDWAVE:
	case ProjectileType::PROJECTILE_REDWAVE:
		aOffsetX += 3.0f;
		break;
	...
	case ProjectileType::PROJECTILE_LOBBED_PUFF:
	case ProjectileType::PROJECTILE_LOBELIA_PETAL:
		aOffsetX += 3.0f;
		aOffsetY += 10.0f;
		aScale = 1.6f;
		break;
