...
{
	return theZombieType == ZombieType::ZOMBIE_GARGANTUAR || theZombieType == ZombieType::ZOMBIE_REDEYE_GARGANTUAR || theZombieType == ZombieType::ZOMBIE_GIGA_FOOTBALL || theZombieType == ZombieType::ZOMBIE_FOOTBALL_GARG;
}
...
void CutScene::StartLevelIntro()
{
	...
	if (mCrazyDaveTime <= 0 && mApp->mGameMode != GameMode::GAMEMODE_INTRO)
	{
		...
		else if (mApp->IsAdventureMode())
		{
			...
			else if (mBoard->mBackground == BackgroundType::BACKGROUND_8)
			{
				aHouseMessage = TodStringTranslate(_S("[PLAYERS_CAMP]"));
			}
			else
			{
				TOD_ASSERT();
